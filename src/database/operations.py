...
def save_crowd_data(people_count, place_name='학생식당'):
    """
    인원수 데이터를 DB에 저장
    """
    # 세션 생성
    ...
    try:
        ...
        # 데이터 생성 - place_name 추가
        new_data = CrowdData(
            place_name=place_name,  # 이 줄 추가
            people_count=people_count,
            occupancy_rate=occupancy_rate,
            crowd_level=crowd_level
        )
        ...
        session.add(new_data)
        session.commit()
        print(f"[{datetime.now()}] {place_name} 데이터 저장 완료: {people_count}명, 혼잡도 {crowd_level}")
        return True
    except Exception as e:
        session.rollback()
        print(f"데이터 저장 오류: {e}")
...
def get_latest_crowd_data(place_name='학생식당'):
    """
    특정 장소의 최신 혼잡도 데이터 조회
    """
    session = Session()
    try:
        latest = session.query(CrowdData).filter(
            CrowdData.place_name == place_name
